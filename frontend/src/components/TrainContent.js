...
class TrainContent extends Component {
    constructor(props) {
        ...
        this.updateKValue = this.updateKValue.bind(this);
    }
    ...
    updateKValue(e) {
        ...
        cluster[e.target.name] = e.target.value;
        this.setState({cluster: cluster}, () => console.log(this.state.cluster));
    }
    ...
    updateModelParams(option) {
        ...
        for (let key in models) {
            ...
            if (models[key] === "K-Means") {
               continue;
            } else if (models[key] === "Bisecting K-Means") {
                continue;
            } else {
                paramValues.push({
        ...
        // console.log(this.state.cluster["kmeans"]);
    ...
    trainModel() {
        ...
        if (this.state.group === "Clustering") {
            ...
            predictData = this.state.paramValues;
        ...
        API.prepareModel(modelData).then((data) => {
            if (data !== 400) {
                ...
                API.predictModel({outputCol: targetColumn, data: predictData, model: this.state.group.toLowerCase()}).then((data) => {
                    if (data !== 400) {
                        console.log(data);
        ...
        return(
            <div className={"col-md-12 no-pad"}>
                ...
                {this.state.group === "Clustering" ?
                    <>
                        <div className={"col-md-12 no-pad"}>
                            <span className={"custom-h2-header"}>Cluster Size</span>
                            <div className={"col-md-12 no-pad medium-bottom-pad medium-top-pad"}>
                                <p className={"col-md-8"}>
                                    You can enter the number of clusters you want to train the model(s) on.
                                </p>
                            </div>
                            {this.state.filterModels !== null ?
                                <>
                                    {this.state.filterModels.map((model, index) => (
                                        <div key={index}>
                                            <span className={"col-md-12 medium-bottom-pad"}><strong>{model.value}</strong></span>
                                            <div className={"col-md-12 no-pad form-group"}>
                                                <label className={"col-md-5"}>K</label>
                                                <div className={"col-md-6"}>
                                                    <input
                                                        type={"text"}
                                                        name={model.value.replace("-","").toLowerCase()}
                                                        value={this.state.cluster["kmeans"]}
                                                        className={"form-control input-sm"}
                                                        placeholder={"Enter the no. of clusters"}
                                                        onChange={(e) => this.updateKValue(e)}
                                                    />
                                                </div>
                                            </div>
                                        </div>
